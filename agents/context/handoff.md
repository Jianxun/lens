...
- T-006 in progress (branch `feature/T-006-retrieval-api`): FastAPI app scaffolded (`backend/main.py`) and retrieval router implemented (`backend/api/retrieval.py`) with `/retrieval/peek` (bin_days histogram over top_k) and `/retrieval/turn/{id}` hydration. `backend/requirements.txt` added. Verification blocked until `SUPER_MIND_API_KEY` is available; currently not set in env.
- 2025-12-25: Loaded `.env` to provide `SUPER_MIND_API_KEY`, ran uvicorn locally, and verified `/retrieval/peek` and `/retrieval/turn/{id}` responses against seeded DB.
...
## Next steps (1–3)
- Keep `.env` sourced for embedding-dependent commands; `SUPER_MIND_API_KEY` required.
- Run uvicorn as needed for further manual tests: `.venv/bin/uvicorn backend.main:app --host 0.0.0.0 --port 8000` then hit `GET /retrieval/peek` and `GET /retrieval/turn/{id}` (sample turn `4ad6f7f2-1442-4fef-a4f3-6109a4d81d4b`).
- If re-running embeddings or ingestion, follow prior instructions; pgweb available on PGWEB_PORT (default 8081) via `docker compose up -d db pgweb`.
